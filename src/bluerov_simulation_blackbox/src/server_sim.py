...
master = mavutil.mavlink_connection('udpin:127.0.0.1:9003')
...
# # ------------------------------angular y (pitch)-------------------------
    if -1.0 <= data.angular.y <= 1.0 and data.angular.y != 0.0:
        diff = np.abs(velocidades - data.angular.y)
        pitchpwm = np.argmin(diff)
        set_rc_channel_pwm(1, pitchpwm + 1100)
        print("Publishing pitch pwm:", pitchpwm + 1100)
    else:
        set_rc_channel_pwm(1, 1500)

    # ------------------------------angular x (roll)-------------------------
    if -1.0 <= data.angular.x <= 1.0 and data.angular.x != 0.0:
        diff = np.abs(velocidades - data.angular.x)
        rollpwm = np.argmin(diff)
        set_rc_channel_pwm(2, rollpwm + 1100)
        print("Publishing roll pwm:", rollpwm + 1100)
    else:
...
def callback_armar(data):
    if data.data:  # Si es True
        armar_rov()
    else:          # Si es False
        desarmar_rov()


def callback_luces_pwm(data):
    pwm_val = data.data
    if 1100 <= pwm_val <= 1900:
        set_rc_channel_pwm(9, pwm_val)
        print(f"PWM luces enviado: {pwm_val}")
    else:
        print("Valor PWM fuera de rango (1100-1900)")

def callback_gripper_pwm(data):
    pwm_val = data.data
    if 1100 <= pwm_val <= 1900:
        set_rc_channel_pwm(3, pwm_val)  
        print(f"PWM gripper enviado: {pwm_val}")
    else:
        print("Valor PWM para gripper fuera de rango (1100-1900)")
...
def listener():
    ...
    rospy.Subscriber('gripper_pwm', Int32, callback_gripper_pwm)
